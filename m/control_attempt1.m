...
energy = nan(n, max(ci), max(ci), length(SC));
totalenergy = zeros(max(ci), max(ci),length(SC));
...
%             energy{j, k, i} = sum(u.^2,2);       % for each node
            energy(keep,j,k,i) = sum(u.^2,2);
            totalenergy(j, k, i) = sum(u(:).^2);  % for brain as a whole
        end
    end
end
save('../output/energy.mat','energy', 'totalenergy','ci')