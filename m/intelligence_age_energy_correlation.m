...
load('../output/nodeAvgADCorrected.mat')
...
for j = 1:numSys
    for k = 1:numSys
        for n = 1:3
            if j ~= k
                keepSubs = ~isnan(wasi_fsiq_prcntile);
                keepNodes = idx_class(:,j,k) == n;
                E = nodeAvgAD_DC_S(keepNodes, keepSubs, j, k);
                rho(keepNodes,j,k) = corr(E', wasi_fsiq_prcntile(keepSubs));
            end
        end
    end
end
...
for i = 1:numNodes
    for j = 1:numSys
        for k = 1:numSys
            if j ~= k
                keepSubs = ~isnan(wasi_fsiq_prcntile);
                E = nodeAvgAD_SC_S(i, keepSubs, j, k);
                rho(i,j,k) = corr(E', wasi_fsiq_prcntile(keepSubs));
            else 
                rho(i,j,k) = NaN;
            end
        end
